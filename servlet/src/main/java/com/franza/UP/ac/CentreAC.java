 ...
 */
public class CentreAC extends AC {

	/**
	 * <p> Implementation of a method that calls the java bean functions according to the user's input and the selected operation on the entity centre. </p>
	...
	public List<TO> handleOperation(HttpServletRequest req, String operation) throws InvalidInputException {
		...
		switch (operation) {
			case "create_centre": {
				list = bean.create(new CentreTO().getFromRequest(req)); 
				break;
			}
			case "read_centre": {
				list = bean.read(new CentreTO().getFromRequest(req).searchAdjust()); 
				break;
			}
			case "update_centre": {
				list = bean.update(new CentreTO().getFromRequest(req).checkKeys()); 
				break;
			}
			case "delete_centre": {
				list = bean.delete(new CentreTO().getFromRequest(req).checkPrimaryKeys()); 
				break;
			}
			default: {
				list = bean.read(new CentreTO().getFromRequest(req));
			}
		}
		return list;
