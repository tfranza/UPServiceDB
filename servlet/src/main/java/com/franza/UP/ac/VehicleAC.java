 ...
 */
public class VehicleAC extends AC {

	/**
	 * <p> Implementation of a method that calls the java bean functions according to the user's input and the selected operation on the entity vehicle. </p>
	...
	public List<TO> handleOperation(HttpServletRequest req, String operation) throws InvalidInputException {
		...
		switch (operation) {
			case "create_vehicle": {
				list = bean.create(new VehicleTO().getFromRequest(req)); 
				break;
			}
			case "read_vehicle": {
				list = bean.read(new VehicleTO().getFromRequest(req).searchAdjust()); 
				break;
			}
			case "update_vehicle": {
				list = bean.update(new VehicleTO().getFromRequest(req).checkKeys()); 
				break;
			}
			case "delete_vehicle": {
				list = bean.delete(new VehicleTO().getFromRequest(req).checkPrimaryKeys()); 
				break;
			}
			default: {
				list = bean.read(new VehicleTO().getFromRequest(req));
			}
		}
		return list;
